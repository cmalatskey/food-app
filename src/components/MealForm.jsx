...
const EMPTY_MEAL = {
  ...
  photo: null,
  proteinSwaps: '',
  ingredients: [], // [{ id, name, isTJ, category }]
  notes: '',
};
...
export default function MealForm({ initial, onSave, onCancel }) {
  ...
  const [newIngCategory, setNewIngCategory] = useState(DEFAULT_CATEGORY);
  ...
  function addIngredient() {
    ...
    setForm((f) => ({
      ...
      ingredients: [
        ...f.ingredients,
        { id: uid(), name: newIngredient.trim(), isTJ: newIngIsTJ, category: newIngCategory },
      ],
    }));
    setNewIngredient('');
    setNewIngIsTJ(true);
    setNewIngCategory(DEFAULT_CATEGORY);
  }
  ...
  const catObj = CATEGORIES.find((c) => c.id === newIngCategory);
    ...
    <div className="fixed inset-0 bg-black/40 z-50 flex items-end">
      <div className="bg-[#fdf6ee] w-full rounded-t-3xl max-h-[92svh] overflow-y-auto pb-24">
        ...
        <div className="flex items-center justify-between px-5 py-2">
          ...
          <button onClick={onCancel} className="text-[#a89080] p-1"><X size={20} /></button>
        </div>
        ...
        <form onSubmit={handleSubmit} className="px-5 flex flex-col gap-5 mt-2">
          ...
          <div className="flex flex-col gap-1">
            <label className="text-xs font-semibold text-[#7a5c48] uppercase tracking-wide">Meal name</label>
            <input value={form.name} onChange={(e) => setForm((f) => ({ ...f, name: e.target.value }))}
              placeholder="e.g. Chicken stir-fry"
              className="bg-[#f5e8d6] rounded-xl px-4 py-3 text-[#3d2b1f] placeholder-[#b8a090] outline-none text-base" required />
          </div>

          {/* Protein swaps */}
          <div className="flex flex-col gap-1">
            <label className="text-xs font-semibold text-[#7a5c48] uppercase tracking-wide">Protein swaps</label>
            <input value={form.proteinSwaps} onChange={(e) => setForm((f) => ({ ...f, proteinSwaps: e.target.value }))}
          ...
          <div className="flex flex-col gap-2">
            <label className="text-xs font-semibold text-[#7a5c48] uppercase tracking-wide">Current vibe</label>
            <VibeRating value={form.vibe} onChange={(v) => setForm((f) => ({ ...f, vibe: v }))} />
          </div>

          {/* Ingredients */}
          <div className="flex flex-col gap-2">
            <label className="text-xs font-semibold text-[#7a5c48] uppercase tracking-wide">Ingredients</label>

            {/* Existing ingredients */}
            <div className="flex flex-col gap-1.5">
              {form.ingredients.map((ing) => {
                const cat = CATEGORIES.find((c) => c.id === ing.category);
                return (
                  <div key={ing.id} className="flex items-center gap-2 bg-[#f5e8d6] rounded-xl px-3 py-2">
                    {cat && <span className="text-base leading-none">{cat.emoji}</span>}
                    <span className="flex-1 text-[#3d2b1f] text-sm">{ing.name}</span>
                    {ing.isTJ && (
                      <span className="text-[10px] bg-[#c9472b] text-white px-2 py-0.5 rounded-full font-semibold">TJ's</span>
                    )}
                    <button type="button" onClick={() => removeIngredient(ing.id)} className="text-[#c08060] p-0.5">
                      <Trash2 size={15} />
                    </button>
                  </div>
                );
            ...
            <div className="flex gap-2">
              ...
              <button type="button" onClick={() => setNewIngIsTJ((v) => !v)}
                className={`px-3 py-2 rounded-xl text-xs font-semibold transition-colors ${newIngIsTJ ? 'bg-[#c9472b] text-white' : 'bg-[#f5e8d6] text-[#a89080]'}`}>
                TJ's
              </button>
            ...
            {/* Category picker for new ingredient */}
            <div className="flex flex-col gap-1.5">
              <span className="text-xs text-[#a89080]">Category for this ingredient:</span>
              <div className="flex gap-2 flex-wrap">
                {CATEGORIES.map((cat) => (
                  <button key={cat.id} type="button" onClick={() => setNewIngCategory(cat.id)}
                    className={`flex items-center gap-1 px-2.5 py-1.5 rounded-xl text-xs font-medium transition-colors ${
                      newIngCategory === cat.id
          ...
          {/* Notes */}
          <div className="flex flex-col gap-1">
            <label className="text-xs font-semibold text-[#7a5c48] uppercase tracking-wide">Notes (optional)</label>
            <textarea value={form.notes} onChange={(e) => setForm((f) => ({ ...f, notes: e.target.value }))}
              placeholder="Any extra tips, variations, or links..." rows={2}
